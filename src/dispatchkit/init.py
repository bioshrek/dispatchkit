...
#
# `auto` lets the runner pick one your account can actually use. Naming a
# specific model here is a promise about availability, which varies by plan and
# by org policy: the first live local run died on a `claude-opus-5` that this
# machine's CLI would not accept, and `doctor` had reported the runner `ok`
# because it checks for the binary and not for the argv it will be given.
model = "auto"
